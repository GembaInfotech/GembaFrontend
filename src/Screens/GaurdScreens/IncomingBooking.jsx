...
const IncomingBooking = () => {
  ...
  const calcMin = (time) => {
    ...
    if (!time || isNaN(new Date(time).getTime())) {
      // Handle invalid or missing checkoutTime
      console.error('Invalid checkoutTime:', time);
      return { days: 0, hours: 0, minutes: 0 };
    }
  
    const exceedTimeInMillis = Math.max(-9999,   new Date(time).getTime()-currentTime);
        ...
        <tbody>
          {data.map(item => (
            <tr key={item._id} onClick={() => openPopup(item)} style={{ cursor: 'pointer' }}>
              ...
              <td className="border px-4 py-2">{new Date(item.timeIn).toLocaleString()}     <span className={`font-bold ${calcMin(item.timeIn).minutes < 0 ? 'text-red-500' : 'text-green-600'}`}>
{ calcMin(item.timeIn).minutes} min</span></td>
